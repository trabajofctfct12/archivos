...
if(isset($_SESSION["Nombre"])){
  ...
  <body class="body1">
    ...
    <div class="reserva">
      ...
      <input class="inputs" type="datetime-local" name="fechainicio" min="2022-01-01">
      <br><br>Tiempo estimado de su estancia:<br><br>
      <input class="inputs" type="number" name="tiempomas" value="" min="1"><br><br>
      Teléfono de contacto:
      <input class="inputs" type="text" name="telf" value=""><br><br>
      Email:
      <input class="inputs" type="email" name="email" value=""><br><br>
...
if($_POST){
  ...
  $mesa = test_input($_POST['select']);
  $fechainicio = test_input($_POST['fechainicio']);
  $tiempomas = test_input($_POST['tiempomas']);
  $telf = test_input($_POST['telf']);
  $email = test_input($_POST['email']);

  $hora=strstr($fechainicio,'T');
  $fecha=strstr($fechainicio,'T',true);
  $horabien=substr($hora,1);
  $usuario=$_SESSION["Nombre"];

  $fechaentera=$horabien." ".$fecha;

  if (isset($_POST['enviar'])) {
    echo "<br><br><br><br>$select <br>$fechainicio<br> $tiempomas $telf $email ";
    echo "<br><br>HORA= $horabien Y FECHA $fecha";
    echo "$usuario";
    //sacamos el id del usuario a través de la sesión
    $idusuario=sacaridusuario($usuario,$conexion);
    //creamos la reserva
    $resultado=crearreserva($idusuario,$mesa,$telf,$email,$fechaentera,$conexion);
